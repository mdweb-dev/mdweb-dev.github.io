...
window.addEventListener("DOMContentLoaded", function () {
  gsap__WEBPACK_IMPORTED_MODULE_1__["default"].registerPlugin(gsap_ScrollTrigger__WEBPACK_IMPORTED_MODULE_2__["default"], SplitText);
  var splitTitleIntro = new SplitText(".js-title-intro", {
    type: "lines,chars"
  });
  var splitTitleSection = new SplitText(".js-title-section", {
    type: 'lines,chars',
    linesClass: 'line'
  });
  var chars = splitTitleSection.chars;
  var correctText = document.querySelector(".js-title-section").textContent;
  //chars.forEach((char) => (char.textContent = ""));

  function animateLetters(chars) {
    var index = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
    if (index >= chars.length) return;
    var charElement = chars[index];
    var targetChar = correctText[index];
  ...
  var jsAboutCompany = document.querySelector('.js-about-company');
  if (jsAboutCompany) {
  ...
  function formatNumber(value, decimals) {
    var s = (+value).toLocaleString('en-US').split(".");
    return decimals ? s[0] + "." + ((s[1] || "") + "00000000").substr(0, decimals) : s[0];
  }
  var interactiveCard = document.querySelectorAll('.interactive-card');
  if (interactiveCard.length) {
    interactiveCard.forEach(function (card) {
      var title = card.querySelector('.interactive-card__title');
